 ...
 */
@Service
@ManagedResource(objectName = CommonConstants.SYS_NAME + "GMSInventoryCostScheduler", description = StringUtils.EMPTY)
public class GMSInventoryCostUpdateScheduler implements RemoteJobServiceExtWithParams {

	private static final Logger log = Logger.getLogger(GMSInventoryCostUpdateScheduler.class);
	...
	public void executeJobWithParams(String jobId, String triggerName, String groupName,
		...
		try {
			...
			log.debug("#####GMS地区价检查的定时任务执行#############");
			calculateWeightedCostApi.batchUpdateGMSInventoryBookCost(dto);
