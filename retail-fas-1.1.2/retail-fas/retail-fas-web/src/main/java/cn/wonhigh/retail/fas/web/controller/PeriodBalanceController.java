 ...
 */
@Controller
@RequestMapping("/period_balance")
@ModuleVerify("30120005")
public class PeriodBalanceController extends BaseController<PeriodBalance> {

	private static final XLogger logger = XLoggerFactory.getXLogger(BaseController.class);
    ...
    public CrudInfo init() {
        return new CrudInfo("period_balance/",periodBalanceManager);
    }
	...
	public  Map<String, Object> queryCompanyPeriodBalances(HttpServletRequest req, Model model) throws ManagerException {
		...
		if("true".equals(req.getParameter("isSubTotal")) || "true".equals(req.getParameter("onlySubTotal"))){
			total = periodBalanceManager.findCompanyPeriodBalanceSubTotalCount(params);
			page = new SimplePage(pageNo, pageSize, (int) total);
			list = periodBalanceManager.findCompanyPeriodBalanceSubTotalPages(page, sortColumn, sortOrder, params);
		}else {
			total = periodBalanceManager.findCompanyPeriodBalanceCount(params);
			page = new SimplePage(pageNo, pageSize, (int) total);
			list = periodBalanceManager.findCompanyPeriodBalancePages(page, sortColumn, sortOrder, params);
		}
		Map<String, Object> obj = new HashMap<String, Object>();
		obj.put("total", total);
		obj.put("rows", periodBalanceManager.setExtendsPeriodBalanceProperties(list));
		List<PeriodBalance> pList = periodBalanceManager.selectTotalRow(page, sortColumn, sortOrder, params);
	...
	protected List<PeriodBalance> queryExportData(Map<String, Object> params) throws ManagerException {
		...
		if (isSubTotal.equals("true") || "true".equals(onlySubTotal)) {
			...
			list = periodBalanceManager.findCompanyPeriodBalanceSubTotalPages(page, orderByField, orderBy, params);
		}else {
			total = periodBalanceManager.findCompanyPeriodBalanceCount(params);
			page = new SimplePage(1, total, (int) total);
			list = periodBalanceManager.findCompanyPeriodBalancePages(page, orderByField, orderBy, params);
		}
		
		list=periodBalanceManager.setExtendsPeriodBalanceProperties(list);
		List<PeriodBalance> pList = periodBalanceManager.selectTotalRow(page, orderByField, orderBy, params);
