     ...
     function insertRowAtEnd(gid,rowData){
    		var tempObj = $('#'+gid);
    		if(rowData){
     ...
     //删除复选框选择的多行
     function removeDataGridAllSelectCommon(dataGridId){
  	   var $dg = $("#" + dataGridId + "");
         var v_all_rows = $dg.datagrid('getChecked');
          if (v_all_rows) {
        	  for(var i = 0;i < v_all_rows.length; i++){
        		  var rowIndex = $dg.datagrid('getRowIndex', v_all_rows[i]);
                  $dg.datagrid('deleteRow', rowIndex);
        	  }
         }
      }
     ...
     // 删除选择的多行数据
     function deleteSelectGridCommon(dataGridId){
  	      var $dg = $("#"+dataGridId+"");
