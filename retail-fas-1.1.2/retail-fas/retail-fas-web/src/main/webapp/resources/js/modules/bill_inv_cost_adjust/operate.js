...
//		setTimeout(function() {
//			if(result){
//				self.preConfirm(statusFlag,myUrl);
//			}
//		}, 2000);
...
// 单据确认2
BillController.prototype.preConfirm = function(statusFlag,myUrl) {
	var self = this;
	// 校验表单数据
	if (!self.validate()) {
		return;
...
// 单据确认3 
BillController.prototype.billConfirm = function(myUrl){
	var self=this;
	var url = myUrl;
	var params = {
		...
		if (data) {
			if (data.errorDefined && data.errorDefined.indexOf('total') > 0) {
				...
				if (isNotBlank(data.errorMessage)) {
					...
					self.setBill(data, 1);
					ctrl.diffMessage(data.diffMessageData);
...
//获取单据状态
BillController.prototype.getBillStatus = function(billNo,shardingFlag) {
	var status='';
...
BillController.prototype.checkListStatus = function(billNo,shardingFlag,flag) {
	...
	switch (flag) {
		case statusFlag.OVER:// 完结操作
			if (!(billStatus == 50)) {
				result=false;
			}
			break;
		case statusFlag.PUT:// 判断是否是制单状态
			if(self.checkDiffrences){
		...
		case "put2":
			...
			break;
		case statusFlag.VERIFY:// 判断是否是审核状态
			if (billStatus != 3) {
				result=false;
			}
			break;
		case statusFlag.CONFIRM:// 判断是否是确认状态
			if (billStatus != 5) {
				result=false;
			}
			break;
		case statusFlag.CANCEL:// 判断作废操作
			if (billStatus != 5 && billStatus != 3) {
				result=false;
			}
			break;
		case statusFlag.SEND:// 判断是否是已发未收
			if (billStatus != 1) {
				result=false;
			}
			break;
		case statusFlag.RECEIVED:// 判断是否是已发未收
			if (billStatus != 2) {
				result=false;
			}
			break;
		case statusFlag.HANDLING:// 判断是否是处理中（开始盘点）状态
			if (billStatus != 6) {
				result=false;
			}
			break;
		case statusFlag.UNPICK:
			if (billStatus != 10) {
				result=false;
			}
			break;
		case statusFlag.SUBMIT:
			if (billStatus != 0 && billStatus != 20) {
				result=false;
			}
			break;
		case statusFlag.WAITVERIFY:
			if (billStatus != 20) {
				result=false;
			}
			break;
		case statusFlag.VERIFYEDWAITSEND:
			if (billStatus != 21) {
				result=false;
			}
			break;
...
BillController.prototype.checkListStatusMessage = function(flag) {
	...
	switch (flag) {
		case statusFlag.OVER:// 完结操作
			result="只有未完结状态的单才能完结,请重新选择单据！";
			break;
		case statusFlag.PUT:// 判断是否是制单状态
			if(self.checkDiffrences){
				result="该单据不是制单或确认状态,请重新选择!";
			}else if(self.checkTransport){
				result="该单据不是制单或提交状态,请重新选择!";
			}else{
				result="该单据不是制单状态,请重新选择!";
			}
			break;
		case "put1":
			result="该单据不是制单或确认状态,请重新选择!";
		...
		case "put2":
			...
			break;
		case statusFlag.VERIFY:// 判断是否是审核状态
			result="该单据不是审核状态,请重新选择!";
			break;
		case statusFlag.CONFIRM:// 判断是否是确认状态
			result="该单据不是确认状态,请重新选择!";
			break;
		case statusFlag.CANCEL:// 判断作废操作
			result="该单据不是审核或者确认状态,请重新选择!";
			break;
		case statusFlag.SEND:// 判断是否是已发未收
			result="该单据不是已发未收状态,请重新选择!";
			break;
		case statusFlag.RECEIVED:// 判断是否是已发未收
			result="只有已收货状态才能确认,请重新选择单据!";
			break;
		case statusFlag.HANDLING:// 判断是否是处理中（开始盘点）状态
			result="单据状态不是处理中,请重新选择!";
			break;
		case statusFlag.UNPICK:
			result="单据状态不是未拣货,请重新选择!";
			break;
		case statusFlag.SUBMIT:
			result="单据状态不是制单或者待审批状态,请重新选择!";
			break;
		case statusFlag.WAITVERIFY:
			result="单据状态不是待审批状态,请重新选择!";
			break;
		case statusFlag.VERIFYEDWAITSEND:
			result="单据状态不是已核待发状态,请重新选择!";
			break;
	}
	return result;
};
	...
	if (atListPage) {
		...
		if (self.bill && isNotBlank(self.bill.billNo)) {
			if (fas.common.checkHasDetails(self.bill.billNo, self.dtlModulePath,self.bill.shardingFlag)) {
				...
				// 判断是否是有尺码横排的导出
				if (self.hasSizeHorizontal) {
					var preCols = [];// 尺码前面的列加上冻结的列
					$.each(self.frozenColumns, function(i, n) {
						preCols.push(n);
					});
					$.each(self.preColNamesDtl, function(i, n) {
						preCols.push(n);
					});
					exportExcel(self._dtlDataGrid, self.bill.billNo, preCols, self.endColNamesDtl, self.dtlModulePath + "do_fas_export.htm" + "?recevie="
							+ self.recevie+'&priceFiled='+self.priceFiled, self.dtlExcelTitle + self.bill.billNo,self.bill.shardingFlag);
				} else {
					self.exportBillDtlExcel();
				}
			}
		} else {
...
BillDetail.prototype.print = function(atListPage,showSalePrice) {
	var self = this;
	var billNo = "";
	if (atListPage) {// atListPage=true 在查询列表
		var checkedData = $('#mainDataGrid').datagrid('getChecked');
	   ...
	   $('#printDialog').show().window('open');		
		
	   function listPrint(r){
		   
			var printFrist= true;
			var i=0;
			var timer = 10000;
			var timeOp=setInterval(function(){
				billNo = checkedData[i].billNo;
				var j = i+1;
				if(j<=checkedData.length-1){
					 var url = self.dtlModulePath + "get_count.json";
					 var params = new Object();
					 params['billNo'] = checkedData[j].billNo;
					 params['shardingFlag'] = checkedData[i+1].shardingFlag;
					 ajaxRequestAsync(url, params, function (totalData) {
					      var total = parseInt(totalData, 10);
					        ...
					        debug("timer="+timer+"total="+total);
					    });
				 }
				 
				self.init();// 在查询列表打印 先初始化明细页的datagrid
				ctrl.bill.shardingFlag = checkedData[i].shardingFlag;
				if (self.hasSizeHorizontal) {
					self.grid.sizeHorizontal({
						billNo : billNo,
						shardingFlag : checkedData[i].shardingFlag 
					});
				}
				if ($.fas.common.checkHasDetails(billNo,self.dtlModulePath, checkedData[i].shardingFlag)) {
					debug("开始打印单号："+billNo);
					print(r,printFrist,true);
					printFrist = false;
				}
				i++;
		...
		if (isNotBlank(billNo)) {
			if ($.fas.common.checkHasDetails(billNo, self.dtlModulePath,self.bill.shardingFlag)) {
				 $('#printDialog').dialog({
					 buttons:[{
							text:'A4规格',
							handler:function(){
	...
	function print(r,printFrist,atListPage){
		var printHeight=0;
		var printWidth=0;
		var printBodyHeight="60%";
		var printFootHeight = "75%";
		var printerType = $('input[name="printerType"]:checked').val();//选择打印机型号
		if (!r){//120*240 规格
			printHeight=1200;
			printWidth=2000;
			printBodyHeight="60%";
			if(printerType==0){// 爱普生型号 横向打印时要单独设置
				printHeight=1400;
				printWidth=2000;
				printBodyHeight="68%";
			}
	    }
		if(printerType==0){//爱普生打印机
		   printFootHeight = "95%";	
		}
	    if (self.hasSizeHorizontal) {
			printDialog({
				'dataGridId' : self._dtlDataGrid,
				'intOrient' : 1,// 0：打印机缺省设置,1：纵向，2：横向
				'type' : 1, // 0：简单打印，1：动态打印
				'needParams' : 0,// 0需要查询参数，1不需要带查询参数
				'queryDataUrl' : self.dtlModulePath + 'getPrintData?priceFiled='+self.priceFiled+'&shardingFlag='+ctrl.bill.shardingFlag,
				'title' : self.printTitle,
				'barcode' : billNo,
				'billNo' : billNo,
				'viewName' : self.printViewName,
				'dtlModulePath' : self.modulePath,
				'preColNamesDtl' : self.preColNamesDtl,
				'endColNamesDtl' : self.endColNamesDtl,
				'printGroupByItemCode':self.printGroupByItemCode,//是否根据item_code 分组打印，合并item_code ,用于配货单打印
				'printGroupByField':self.printGroupByField,//是否根据item_code 分组打印，合并item_code ,用于配货单打印
				'printHeight':printHeight,
				'printWidth':printWidth,
				'printBodyHeight':printBodyHeight,
				'printFrist':printFrist,
				'atListPage':atListPage,
				'printFootHeight':printFootHeight,
				'isAutoSwitchPrintDirection':self.isAutoSwitchPrintDirection,
				'isShowTotalQtyInEnd':self.isShowTotalQtyInEnd
			});
		} else {
			var url = self.dtlModulePath;
			if (self.getPrintDataMethod) {
				url += self.getPrintDataMethod;
			} else {
				url += "getPrintData";
			}
			printDialog({
				...
				'printWidth':printWidth,
				'printBodyHeight':printBodyHeight,
				'printGroupByItemCode':self.printGroupByItemCode,//是否根据item_code 分组打印，合并item_code ,用于拣货单
				'printGroupByField':self.printGroupByField,//是否根据item_code 分组打印，合并item_code ,用于拣货单
				'printFrist':printFrist,
				'atListPage':atListPage,
				'printFootHeight':printFootHeight,
				'printFormatterForReturn':self.printFormatterForReturn//客残退厂发货单打印格式控制	
			});
		}
	}
};
...
BillList.prototype.onMainDblClickRow = function(index,row){
	this.selectBill(index, row);
	this._tab.tabs('select', '单据明细');
};
		...
		onLoad : function() {
			...
			grid.datagrid({
				...
				onDblClickRow : function(index, row) {
					self.onMainDblClickRow(index,row);
				}
...
BillList.prototype.batchConfirm = function(billStatus, myUrl) {
	...
	if (flag) {
		if (isNotBlank(isNotMakeBillNos)) {
			if(self.ctrl.checkTransport){
				showWarn("其中" + isNotMakeBillNos + "为非制单和提交状态的单据，请选择制单状态的单据进行操作！");
			}else{
...
//									showSuc('确认成功!');
					...
					showError(item.billNo+"库存不足，不允许负库存出库！");
					return false;
				}
			});
...
BillList.prototype.batchCancel = function(checkFlagFlag) {
	if(!checkFlagFlag){
		checkFlagFlag=statusFlag.CANCEL;
	}
	var self = this;
	var grid = $('#' + self.gridId);
	var rows = grid.datagrid('getRows');
	...
	$.messager.confirm("确认", "确定要作废这" + rowObj.length + "条单据?", function(r) {
		if (r) {
			// $.messager.progress({msg:'处理中',interval:1000});
			var inventoryFlag = false;
			var dataObject = [];
