...
function BillBalanceController(){
    ...
    if(typeof BillBalanceController.prototype._initialized == 'undefined'){
        ...
        BillBalanceController.prototype.getErrorMessage = function(currStatus ,operStatus){
        	...
        	if(_self.params.balanceType == 2){
        		if($('#isArea').val() !="true"){// 总部
        			...
        			}else if(operStatus == 99 && currStatus!=2){// 打回
        				return backAreaMessage;
        			}
        		}
        	}else{
        		...
        		}else if(operStatus == 99 && currStatus!=1){// 总部财务打回
        			return fasBackMessage;
        		}
        	}
        	return "";
        };
    	...
    	BillBalanceController.prototype.doSubmit = function(){ // 提交
		...
		BillBalanceController.prototype.exportBalance = function (){
		    ...
		    fromObj.form('submit', {
				url: _self.params.balanceExportUrl,
				onSubmit: function(param){
					param.balanceType = _self.params.balanceType;
					param.billNo = billNo;
    	...
    	BillBalanceController.prototype.toCreateBalance = function(){
			ygDialog({
				title : '选择关联单据',
				href : _self.params.toCreateBalanceUrl +'?balanceType='+_self.params.balanceType,
				width : 1000,
				modal : true,
				showMask : true,
				isFrame : true,
				...
				buttons: [{
		            ...
		            handler: function(dialog) {
		            	if(_self.validateBalance()){
			            	...
			            	params.balanceType = _self.params.balanceType;
			            	ajaxRequestAsync(_self.params.createBalanceUrl, params ,function(data){
			            		if(data){
					            	dialog.close();
									showSuc('保存成功');
    	...
    	BillBalanceController.prototype.toBalanceAdjust = function(){
			...
			ygDialog({
				title : '扣项调整',
				href : _self.params.toAdjustBalanceUrl +'?balanceType='+_self.params.balanceType,
				width : 1000,
				height : 'auto',
				isFrame : true,
				modal : true,
				...
				buttons: [{
		            ...
		            handler: function(dialog) {
		            	...
		            	params.checkedRows = JSON.stringify(checkedRows);
		            	params.balanceType = _self.params.balanceType;
		            	ajaxRequestAsync(_self.params.adjustBalanceUrl, params, function(data){
		            		if(data){
		            			 dialog.close();
								showSuc('保存成功');
								_self.loadMainData(data);
    	...
    	BillBalanceController.prototype.saveDtl = function(){// 保存明细 
    		if($('#deductionDataGrid').length > 0 && _self.endEdit()){
        		...
        		ajaxRequestAsync(_self.params.saveDeductionUrl,data,function(result){
        			if(result){
        			  	$( '#deductionDataGrid').datagrid('acceptChanges');
        			  	$( '#deductionDataGrid').datagrid( 'options' ).queryParams= {balanceNo:billNo};
    				    $( '#deductionDataGrid').datagrid( 'options' ).url = _self.params.listDeductionUrl;
