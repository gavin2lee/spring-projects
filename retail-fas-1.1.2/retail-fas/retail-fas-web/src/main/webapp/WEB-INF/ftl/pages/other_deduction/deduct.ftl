			...
			<div data-options="region:'north',border:false" >
				<div class="search-div">
			      	<form name="searchForm" id="searchForm" method="post">
			      		<input type="hidden" id="atoa" name="atoa" value="${atoa}">
			           ...
			           columnsJsonList="[
			                 ...
			                 {field : 'buyerName', title : '公司', width : 200, 
			                 	editor:{type:'test_combogrid',
			                 			options:{type:'company',required:true,datagridId:'dtlDataGrid',valueField:'buyerNo'}},
			                 ...
			                 {field : 'salerName', title : '供应商', width : 200, 
			                 	editor:{type:'test_combogrid',
			                 			options:{type:'supplier',required:true,datagridId:'dtlDataGrid',valueField:'salerNo'}}, 
			                 align:'left',halign:'center'},
							 ...
							 {field : 'fineAmount',title:'扣项金额',width:125,align:'right',halign:'center', 
			 				 ...
			 				 {field : 'balanceNo',title:'结算单号',width:180,align:'left',halign:'center', formatter: function(value,row,index){
								var title;
								if($('#balanceType').val() == 6) {
									title = 'AM-结算单';
								}else if ($('#balanceType').val() == 13) {
									if($('#atoa').val() == 'yf'){
										title = 'AP-结算单查询';
									}else {
										title = 'HP-结算单';
