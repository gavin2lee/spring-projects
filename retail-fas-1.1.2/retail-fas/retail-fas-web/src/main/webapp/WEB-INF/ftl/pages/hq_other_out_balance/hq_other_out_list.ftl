...
<head>
<meta charset="utf-8">
<title>总部其他出库明细</title>
<#include  "/WEB-INF/ftl/common/header.ftl" >
<script type="text/javascript" src="${resourcesUrl}/fas/resources/js/common/area_detail.js?version=${version}"></script>
<script type="text/javascript" src="${resourcesUrl}/fas/resources/js/modules/hq_other_stock_out/hqOtherStockOut.js?version=${version}"></script>
						...
						<table class="form-tb">
							...
							<tbody>
								...
								<tr>
									...
									<th>商品编码： </th>
									<td>
										<input class="easyui-itemCommon" id="itemName" /><input type="hidden" name="itemSql" id="itemNo"/>
								...
								<tr>
									...
									<td>
										...
										data-options="queryUrl: BasePath + '/base_setting/company/list.json?params=groupLeadRole',inputNoField:'buyerNoId',inputNameField:'buyerNameId',inputWidth:160"/>
									...
									<th>单据类型：</th>
						    		<td>
						    			<select class="easyui-combobox"  name="billType" id="billType" data-options="editable:false,valueField:'code',textField:'name',width:'auto',url:BasePath + '/area_balance_common/getBillType?showType=SALEOUT'"/>
						    		</td> -->
									<th>结算单号：</th>
									<td>
		              ...
		              isHasToolBar="false" onClickRowEdit="false" pagination="true" pageSize="20"
			           ...
			           columnsJsonList="[
			                  ...
			                  {field : 'balanceNo', title : '结算单号', width : 150,align:'center',halign:'center', formatter: function(value,row,index){
								 return billNoMenuRedirect.billNoMenuFormat(value,row,index,'HO-结算单');
							  }}
		                  ]" 
                 />
			</div>
