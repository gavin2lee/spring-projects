...
<#macro subdatagrid
	...
	subPagination="false" subHeight='200' subSingleSelect='true'>
	        ...
	        <#if onClickRowAuto='true'> 
		     ...
		     <#if loadSubGridUrl !=''> 	
	            ...
	            detailFormatter:function(index,row){
	                return '<div style="padding:2px;height:${subHeight}px"><table class="ddv"></table></div>';
	            },
	            onExpandRow: function(index,row){
	                var ddv = $(this).datagrid('getRowDetail',index).find('table.ddv');
	                var url = "<@s.url loadSubGridUrl/>";
	                if(typeof buildSubgridUrl != 'undefined' 
	                	&& buildSubgridUrl != null
	                	&& typeof buildSubgridUrl == 'function') {
	                	url += buildSubgridUrl(row, false, index);
	                } else {
	                	url += "?billNo=" + row.billNo;
	                }
	                ddv.datagrid({
